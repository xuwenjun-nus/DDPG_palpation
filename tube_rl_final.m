...
disturbance=1e-6.*ones(nbFeatures,1);
...
Q = zeros(M,n_iter+1);
for i=1:M
    Q(1:(end+1-i),1) = Q(1:(end+1-i),1) + 0.05*abs(sqrt((zz(i)-43)^2+(yy(i)-20)^2)-30) + 0.05*abs(xx(i));
end
Q(:,1) = exp(-Q(:,1)./M);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Do the iterations
for iter=1:n_iter
    ...
    for i=1:M
        Q(1:(end+1-i),iter+1) = Q(1:(end+1-i),iter+1) + 0.05*abs(sqrt((zz(i)-43)^2+(yy(i)-20)^2)-30) + 0.05*abs(xx(i));
    end
...
%material('dull');
material('shiny');
plot3(cx,cy,cz+13,'r','LineWidth',3)
...
for j=1:250:1001
    plot3(position_z(1:10:end,j),position_y(1:10:end,j),position_x(1:10:end,j)+13,'bo');
    hold on;
    ly=linspace(0,position_y(end,j),10);
    ln=length(ly);
    lz=position_z(end,j)*ones(1,ln);
    lx=13*ones(1,ln);
    plot3(lz,ly,lx,'bo');
    hold on;
end

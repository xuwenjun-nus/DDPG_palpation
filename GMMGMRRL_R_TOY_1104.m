...
variance = 0.005.*ones(nbFeatures,1);
...
zz=EndPos(:,3);
for i=1:M
    plot3(xx(i),yy(i),zz(i),'ro','MarkerSize',4);
    hold on;
    pause(0.025);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iter=1:n_iter
    ...
    if iter~=n_iter
        param_rm(:,iter+1) = param_rm(:,iter+1) + variance.^.5.*randn(nbFeatures,1);
        param_lm(:,iter+1) = param_lm(:,iter+1) + variance.^.5.*randn(nbFeatures,1);
        param_zb(:,iter+1) = param_zb(:,iter+1) + variance.^.5.*randn(nbFeatures,1);
    end
